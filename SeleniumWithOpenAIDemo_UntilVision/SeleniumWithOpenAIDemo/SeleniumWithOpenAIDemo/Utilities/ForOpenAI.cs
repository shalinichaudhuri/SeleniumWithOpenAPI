...
public class ForOpenAI
{
    ...
    public static async Task<string> PassImageToAiAsync(byte[] beforeState, byte[] afterState)
    {
        ...
        [
            new UserChatMessage(
                ChatMessageContentPart.CreateTextPart($"Compare two images and get ONLY the differences in both as JSON output, use the following JSON schema {schema}"),
                ChatMessageContentPart.CreateImagePart(new BinaryData(beforeState), "image/png", ChatImageDetailLevel.Auto),
                ChatMessageContentPart.CreateImagePart(new BinaryData(afterState), "image/png", ChatImageDetailLevel.Auto)
                )
        ];
